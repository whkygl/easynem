...
#' ```
#'
#' @usage calc_ef2(data, .group1, .group2)
#'
...
#' nem_index
calc_ef2 <- function(data, .group1, .group2){
  # data = hehe
  # .group = "con_crop"
  .ef = methods::new("ef2")
  ...
  meta = meta[,c("SampleID", .group1, .group2)]
  alltab2 = merge(meta, alltab2, by = "SampleID")
  alltab2 = tibble::as_tibble(alltab2)
  .ef@result = alltab2
  .ef@tab = data@tab
  .ef@tax = data@tax
  .ef@meta = merge(data@meta, alltab2[,c(-2,-3)], by = "SampleID")
  return(.ef)
}
