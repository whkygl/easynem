...
calc_nemindex <- function(data, ...){
  ...
  nemdbgenus <- nemdbgenus |>
    dplyr::mutate(Feeding = dplyr::case_when(
      ...
      feeding == 7 ~ "d",
      feeding == 8 ~ "o",
      TRUE ~ NA_character_  # 默认值，处理其他未定义的情况
    ))
  ...
  index1 <- function(otutab,taxonomy){
    ...
    #保留数据框FunctionalGuild列中含有f,b,s,p,e,o字母的行
    alltab = alltab[grepl("f|b|s|p|e|o", alltab$FunctionalGuild), ]
    #删除数据框FunctionalGuild列中包含ap的行
    ...
    cpralltab = cpralltab[complete.cases(cpralltab),]
...
hehe <- hehe|>trans_name(Genus)
hehe <- hehe@tab
hehe <- as.data.frame(hehe)
rownames(hehe) <- hehe[,1]
hehe <- hehe[,-1]
hehe <- t(hehe)
write.csv(hehe, "Nemaplex_NINJA_test.csv")
